...
-- Utility Functions Reconstruction
local Utils = {}
...
Logic.WeakestPlayer = TEMP_NO_VIRTUALIZE(function(self)
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            local data = {
                Player = Utils:GetHumanoidRootPart(plr)
            }
...
Logic.Angle = TEMP_NO_VIRTUALIZE(function(self, mode, target)
    local data = {
        LocalPlayer = Utils:GetHumanoid(LocalPlayer.Character)
    }
    local cf
    local diff
    if not target then
    ...
    elseif mode == "Random" and target then
        local choices = {
            {
                Value = Logic:Angle("Forward", target),
                Weight = 60
            },
            {
                Value = Logic:Angle("Backward", target),
                Weight = 30
            },
            {
                Value = Logic:Angle("Upward", target),
                Weight = 10
            }
        }
...
Logic.Untarget = TEMP_NO_VIRTUALIZE(function(self, obj, _) -- Second arg unused
...
Logic.VisualizeBall = TEMP_NO_VIRTUALIZE(function(self, ball, info)
    if Config.Visualize then
        ...
        for _, child in pairs(visual:GetChildren()) do
            child:Destroy()
        end
...
Logic.RealBall = TEMP_NO_VIRTUALIZE(function(self, specificBall)
    local function SetupBall(obj)
        ...
        local instance = setmetatable({
            ...
            Target = LocalPlayer,
            TargetCount = 0,
            ZoomiesVelocity = Vector3.zero,
            Velocity = Vector3.zero,
            Position = Vector3.zero,
            LastPosition = Vector3.zero,
            LastZoomiesVelocity = Vector3.zero,
            LastVelocity = Vector3.zero,
            HighestVelocity = Vector3.zero,
            LastParriedVelocity = Vector3.zero,
            FluctuationLastVelocity = Vector3.zero,
            FluctuateBallLastPosition = Vector3.zero,
            ZoomiesFluctuateBallLastPosition = Vector3.zero,
            FluctuationVelocity = Vector3.zero,
            ZoomiesFluctuationVelocity = Vector3.zero,
            FluctuationZoomiesVelocity = Vector3.zero,
            ZoomiesFluctuationZoomiesVelocity = Vector3.zero,
            Parried = false,
            InitialDot = 0,
            Dot = -1.0,
            FluctuationDot = -1.0,
            ZoomiesFluctuationDot = -1.0,
            ZoomiesDot = -1.0,
            FluctuationZoomiesDot = -1.0,
            ZoomiesFluctuationZoomiesDot = -1.0,
            DotTarget = -1.0,
            LastDot = -1.0,
            AntiCurve = true,
            TargetInterval = os.clock(),
            LastTargetInterval = 0,
            Animators = {}
        }, {
            BallClass == BallClass
        })
        ...
        local function UpdateTargets()
            ...
            if instance.Target ~= prevTarget or instance.TargetCount >= 1 then
                ...
                if #targetHistory > 2 then
                    table.remove(targetHistory, #targetHistory - 2)
                end
                    ...
                    for _, child in pairs(obj:GetChildren()) do
                        if child:IsA("LinearVelocity") then
                            if child.Name == "zoomies" then
                                ...
                                child:GetPropertyChangedSignal("VectorVelocity"):Connect(function()
                                    instance.Zoomies = child
                                end)
                            end
                        end
        ...
        local function RenderLoop(dt)
            ...
            if Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and instance.Object and instance.Object:GetAttribute("realBall") and instance.Object.Parent then
                ...
                if instance.Dot >= 0 and instance.LastDot >= 0 then
                    instance.DotInterval = instance.Dot - instance.LastDot
                end

                if instance == State.RealBall then
                    State.RealBall = nil
                end
                ...
                if instance.ZoomiesFluctuated and instance.ZoomiesProgressionParryable and not instance.ZoomiesDecreaseParryable then
                    if ((instance.ZoomiesVelocity.Magnitude - instance.LastZoomiesVelocity.Magnitude) < instance.LastZoomiesVelocity.Magnitude * -0.0125) or (instance.ZoomiesProressionTimestamp and os.clock() - instance.ZoomiesProressionTimestamp >= 0.175) or ((function()
                        ...
                        if dist <= instance.Velocity.Magnitude * ((ping + dt) * 3) then
                            return true
                        end
                    end)()) then
                        if not instance.DebounceZoomiesDecreaseParryable then
                            task.spawn(function()
                                ...
                                if Config.Visualize then
                                        ...
                                        for i = 65.0, 96 do
                                            ...
                                            if State.VisualBallsCache[i] then
                                                ...
                                                task.spawn(function()
                                                    ...
                                                    while not timer() do
                                                        if loopId ~= State.VisualBallsLoopId3 then
                                                            break
                                                        end
                                                        task.wait()
                                                    end
                                                    if timer() then
                                                        State.VisualBallsCache[i].Transparency = 1
                                                    end
                                                end)
                                            else
                                                State.VisualBallsCache[i] = Logic:VisualizeBall(obj, {
                                                    Cache = true,
                                                    Transparency = instance.Target == LocalPlayer and 0 or 0.8,
                                                    Position = pos,
                                    ...
                                    Logic:VisualizeBall(obj, {
                                        Duration = 1,
                                        Transparency = instance.Target == LocalPlayer and 0 or 0.8,
                                        Position = vizPos,
                                        Size = Vector3.new(3, 3, 3),
                                        Color = Color3.fromRGB(0, 254, 220)
                                    })
                                end
                            end)
                        end
                                ...
                                for i = 1.0, 32 do
                                    ...
                                    if State.VisualBallsCache[i] then
                                        ...
                                        task.spawn(function()
                                            local timer = Utils:Timer(1)
                                            while not timer() do
                                                if loopId ~= State.VisualBallsLoopId1 then break end
                                                task.wait()
                                            end
                                            if timer() then
                                                State.VisualBallsCache[i].Transparency = 1
                                        ...
                                        State.VisualBallsCache[i] = Logic:VisualizeBall(obj, {
                                            ...
                                            Position = pos,
                                            Size = Vector3.new(1, 1, 1),
                                            Color = Color3.fromRGB(254, 0, 254)
                                        })
                                    end
                                end
                            end
                                ...
                                for i = 33.0, 64 do
                                    ...
                                    if State.VisualBallsCache[i] then
                                        ...
                                        task.spawn(function()
                                            local timer = Utils:Timer(1)
                                            while not timer() do
                                                if loopId ~= State.VisualBallsLoopId2 then break end
                                                task.wait()
                                            end
                                            if timer() then
                                                State.VisualBallsCache[i].Transparency = 1
                ...
                if not Config.AwaitParry then
                    ...
                    if instance.Target and instance.Target ~= LocalPlayer and ((Humanoids.LocalPlayer.RootPart.Position - instance.Position).Magnitude <= 10 and instance.DistanceFromTarget and instance.DistanceFromTarget <= 10) then
                    end
        ...
        connections[#connections + 1] = obj.Destroying:Connect(function()
            Cleanup()
        end)
        return instance
    end
    ...
    if specificBall then
        if specificBall:GetAttribute("realBall") == true then
            return SetupBall(specificBall)
        end
    else
        for _, ball in pairs(BallsFolder:GetChildren()) do
            if ball:GetAttribute("realBall") == true then
                return SetupBall(ball)
            end
        end
    end
end)
...
Logic.PlayersScreenPoint = TEMP_NO_VIRTUALIZE(function(self)
    ...
    for _, char in pairs(Workspace.Alive:GetChildren()) do
        ...
        if root then
            points[char.Name] = Camera:WorldToScreenPoint(root.Position)
        end
    end
    return points
end)
...
-- Config Init
if not (isfile("Infinixity") or isfolder("Infinixity")) then
    makefolder("Infinixity")
end
local ConfigPath = ("%s/Blade_Ball"):format("Infinixity")
if not (isfile(ConfigPath) or isfolder(ConfigPath)) then
    ...
    if loaded.Info and loaded.Info.Version and loaded.Info.Version == SettingsData.Info.Version then
        for k, v in pairs(loaded) do
            for key, val in pairs(v) do
                if type(SettingsData[k]) == "table" then
                    SettingsData[k][key] = val
                end
            end
        end
    end
...
if getgc and getgc() then
    for _, v in pairs(getgc()) do
        if type(v) == "function" then
            local info = debug.info(v, "slnaf") -- source, line, name, args, upvalues, func
...
if typeof(State.ParryAttemptKey) ~= "string" and typeof(RemoteSignals.ParryAttempt) ~= "Instance" then
    Library:Notify({
        ...
        Title = "Cannot use Auto Parry",
        Body = "Executor does not support function.",
        Duration = 15,
        Buttons = {}
    })
    return
    ...
    for _, ballWrapper in pairs(State.Balls) do
        if ballWrapper and ballWrapper.Object then
            if ballWrapper.Object.Name == inst:GetAttribute("Ball") then
                ...
                task.delay(inst:GetAttribute("TornadoTime"), function()
                    table.remove(ballWrapper.Animators, idx)
                end)
            end
        end
    end
...
Workspace.ChildAdded:Connect(TEMP_NO_VIRTUALIZE(function(child)
    if child.Name:lower() == "clash" and child:IsA("BasePart") then
        ...
        conn = child.Destroying:Connect(function()
            ...
            if State.ClashObject and State.ClashObject <= 0 then
                State.ClashObject = nil
            end
        end)
    end
end))
...
local function SetupAlive(child)
    if not State.AliveData[child] then
        State.AliveData[child] = {
            ParryFrequency = math.huge,
            LastParried = math.huge
        }
    end
end

local function RemoveAlive(child)
    if State.AliveData[child] then
        State.AliveData[child] = nil
    end
end
    ...
    do
            ...
            Function = TEMP_NO_VIRTUALIZE(function(val)
                ...
                if not State.Dungeon then
                    for _, v in pairs(Workspace.Map:GetChildren()) do
                        if v.Name:lower():match("^dungeon") then
                            State.Dungeon = true
                            break
                        end
                    end
                ...
                if not isSpamming then
                    if State.Dungeon then
                        ...
                        parryArgs[1] = 0.5 -- Adjusted value based on obs
                        ...
                        if targetPlayer then
                            ...
                            if not success then
                                targetPlayer = Utils:GetHumanoidRootPart(targetPlayer)
                            end
                        ...
                        task.spawn(function()
                            ...
                            repeat
                                task.wait()
                            until timer() or not Config.Parry
                            if Config.Parry then
                                Config.Parry = false
                            end
                        end)
                    end
        ...
        GUI.Combat.Toggle["Auto-Parry"] = Section:CreateToggle({
            ...
            Function = function(bool)
                ...
                if SignalWrapper["Auto-Parry"] then
                    SignalWrapper["Auto-Parry"]:Disconnect()
                end
                local debounce = false
                SignalWrapper["Auto-Parry"] = RunService.PreRender:Connect(TEMP_NO_VIRTUALIZE(function(dt)
                    if not SettingsData.Toggle["Auto-Parry"] then
                        SignalWrapper["Auto-Parry"]:Disconnect()
                        return
                    end
                    ...
                    if Logic:Alive(LocalPlayer) and realBall and realBall.Object and realBall.Target == LocalPlayer and not realBall.Parried and realBall.Velocity.Magnitude >= 0 and #realBall.Animators <= 0 and Humanoids.LocalPlayer and LocalPlayer.Character.Parent == Workspace.Alive and ((realBall.ZoomiesFluctuated and ((function()
                        if realBall.ZoomiesDot >= 0 then
                            ...
                            if dist <= math.clamp(realBall.Velocity.Magnitude, 17, 17) then
                                return true
                            end
                        end
                        ...
                        task.spawn(function()
                            ...
                            repeat
                                task.wait()
                            until timer() or not (realBall and realBall.Parried)
                            if realBall and realBall.Parried then
                                realBall:SetParry(false, "Due")
                            end
                        end)
                        debounce = false
                    end
        ...
        GUI.Combat.Toggle["Auto-Spam-Parry"] = Section:CreateToggle({
            ...
            Function = function(bool)
                ...
                if SignalWrapper["Auto-Spam-Parry"] then
                    SignalWrapper["Auto-Spam-Parry"]:Disconnect()
                end
                SignalWrapper["Auto-Spam-Parry"] = RunService.PostSimulation:Connect(TEMP_NO_VIRTUALIZE(function(step)
                    if not SettingsData.Toggle["Auto-Spam-Parry"] then
                        SignalWrapper["Auto-Spam-Parry"]:Disconnect()
                        return
                    end
                    local ping
                    ...
                    if not Config.Spam and Logic:Alive(LocalPlayer) and realBall and realBall.Object and realBall.Velocity.Magnitude > 0 and realBall.Target == LocalPlayer and #realBall.Animators <= 0 and Humanoids.LocalPlayer and LocalPlayer.Character.Parent == Workspace.Alive and (State.ForceSpam or (realBall.ZoomiesFluctuated and (Utils:TowardsPosition(Humanoids.LocalPlayer.RootPart.Position, realBall.Position, realBall.Velocity) >= 0)) or (realBall.ZoomiesFluctuated and ((function()
                        ...
                        local function SpamLogic(dt)
                            local p
                            pcall(function()
                                p = (LocalPlayer:GetNetworkPing() * 2) or (game.Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000)
                            end)
                            ...
                            if (Logic:Alive(LocalPlayer) and SettingsData.Toggle["Auto-Parry"] and SettingsData.Toggle["Auto-Spam-Parry"] and ((State.LocalParryFrequency and State.LocalLastParried and State.LocalParryFrequency <= 0.225 + (p + dt) and os.clock() - State.LocalLastParried <= 0.325 + (p + dt)) or (State.EstimatedTimeArrival and State.LastEstimatedTimeArrival and State.LastEstimatedTimeArrivalTimestamp and (State.EstimatedTimeArrival <= 0.225 + (p + dt) and State.LastEstimatedTimeArrival <= 0.225 + (p + dt) and State.EstimatedTimeArrival - State.LastEstimatedTimeArrival < 0.225 + (p + dt)) and os.clock() - State.LastEstimatedTimeArrivalTimestamp <= 0.35))) then
                                if not Config.Spam then
                                    if (State.EstimatedTimeArrival and State.LastEstimatedTimeArrival and State.LastEstimatedTimeArrivalTimestamp) then
                                        warn("SPAM-ACTIVATE", State.EstimatedTimeArrival, State.LastEstimatedTimeArrival)
                                    end
                                end
                                State.Spamming = true
                                Config.Spam = true
    ...
    do
        ...
        GUI.Settings.Toggle["Auto-Config"] = Section:CreateToggle({
            ...
            Function = function(bool)
                ...
                if SignalWrapper["Auto-Config"] then
                    SignalWrapper["Auto-Config"]:Disconnect()
                end
                SignalWrapper["Auto-Config"] = RunService.PostSimulation:Connect(TEMP_NO_VIRTUALIZE(function()
                    if not SettingsData.Toggle["Auto-Config"] then
                        SignalWrapper["Auto-Config"]:Disconnect()
                        return
                    end
                    local ping
                    ...
                    if isDungeon then
                        GUI.Combat.Slider["Range"]:Set("Value", 0.3)
                        GUI.Combat.Slider["Direct-Point"]:Set("Value", -0.25)
                    elseif GUI.Combat and GUI.Combat.Slider["Range"] then
                        GUI.Combat.Slider["Range"]:Set("Value", 0.35 + 0.05 * (ping / 0.05))
                        GUI.Combat.Slider["Direct-Point"]:Set("Value", 0)
                    end
                end))
            end
        })
            ...
            Function = function(bool)
                ...
                if not State["Block-Spam-Parry"] then
                    ...
                    local function BindBlock(button1, button2)
                        -- Logic to detect blocking UI state (gradients)
                        ...
                        conns[#conns + 1] = button1.InputBegan:Connect(TEMP_NO_VIRTUALIZE(function(input)
                            if SettingsData.Toggle["Block-Spam-Parry"] and active and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
                                ...
                                SpamLoop()
                                RunService:BindToRenderStep("SpamParryToggle", 0, SpamLogic)
                                -- Bind other runservice events...
                            end
                        end))
                        conns[#conns + 1] = button1.MouseButton1Down:Connect(function()
                            if SettingsData.Dropdown["Block-Mode"] == "Hold" then active = true end
                        end)
                        conns[#conns + 1] = button1.MouseButton1Up:Connect(function()
                            if SettingsData.Dropdown["Block-Mode"] == "Hold" then active = false end
                        end)
                    end
                    BindBlock(Logic:BlockButton(), Logic:AbilityButton())
                    PlayerGui.ChildAdded:Connect(function(c)
                        if c.Name == "Hotbar" then
                            BindBlock(c:WaitForChild("Block"), c:WaitForChild("Ability"))
                        end
                    end)
