--[[
    ...
    Shared Core Module
    Contains: Services, Utils, Logic, State, Config, References, RemoteSignals,
              SignalWrapper, SettingsData, event wiring, and reusable callback factories.

    Usage:
        local Core = loadstring(game:HttpGet("<raw-url>/shared/core.lua"))()
        local result = Core.init({ notify = function(title, text, duration) ... end })
        -- result contains all shared modules; nil if init failed.
]]

local CoreModule = {}

function CoreModule.init(opts)
    opts = opts or {}
    local notifyFn = opts.notify or function() end
    ...
    Logic.RealBall = TEMP_NO_VIRTUALIZE(function(self, specificBall)
        local function SetupBall(obj)
            ...
            local function RenderLoop(dt)
                ...
                Humanoids.From = instance.From and Utils:GetHumanoid(instance.From)
                Humanoids.LastTarget = instance.LastTarget and Utils:GetHumanoid(instance.LastTarget)
                Humanoids.LastActualTarget = instance.LastActualTarget and Utils:GetHumanoid(instance.LastActualTarget)

                if instance.Object and instance.Object.Parent then
                    local ball = instance.Model and instance.Model:FindFirstChild("Body") or instance.Object
                    instance.Position = ball.Position
                    instance.Velocity = ball.AssemblyLinearVelocity
                    instance.ZoomiesVelocity = instance.Zoomies and instance.Zoomies.VectorVelocity or instance.Velocity

                    instance.Dot = Utils:TowardsPosition(instance.Position, Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and Humanoids.LocalPlayer.RootPart.Position or instance.Position, instance.Velocity)
                    instance.ZoomiesDot = Utils:TowardsPosition(instance.Position, Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and Humanoids.LocalPlayer.RootPart.Position or instance.Position, instance.ZoomiesVelocity)
                    instance.DotTarget = Humanoids.Target and Humanoids.Target.RootPart and Utils:TowardsPosition(instance.Position, Humanoids.Target.RootPart.Position, instance.Velocity) or -1

                    instance.FluctuationVelocity = instance.Velocity - instance.LastVelocity
                    instance.ZoomiesFluctuationVelocity = instance.ZoomiesVelocity - instance.LastZoomiesVelocity
                    instance.FluctuationZoomiesVelocity = instance.Velocity - instance.LastZoomiesVelocity
                    instance.ZoomiesFluctuationZoomiesVelocity = instance.ZoomiesVelocity - instance.LastVelocity

                    instance.FluctuationDot = Utils:TowardsPosition(instance.Position, Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and Humanoids.LocalPlayer.RootPart.Position or instance.Position, instance.FluctuationVelocity)
                    instance.ZoomiesFluctuationDot = Utils:TowardsPosition(instance.Position, Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and Humanoids.LocalPlayer.RootPart.Position or instance.Position, instance.ZoomiesFluctuationVelocity)
                    instance.FluctuationZoomiesDot = Utils:TowardsPosition(instance.Position, Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and Humanoids.LocalPlayer.RootPart.Position or instance.Position, instance.FluctuationZoomiesVelocity)
                    instance.ZoomiesFluctuationZoomiesDot = Utils:TowardsPosition(instance.Position, Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart and Humanoids.LocalPlayer.RootPart.Position or instance.Position, instance.ZoomiesFluctuationZoomiesVelocity)

                    if instance.ZoomiesFluctuationDot >= 0 then instance.ZoomiesFluctuated = true end
                    if instance.FluctuationDot >= 0 then instance.Fluctuated = true end

                    if instance.ZoomiesFluctuated then
                        if instance.ZoomiesDot >= 0 and not instance.ZoomiesParryable then instance.ZoomiesParryable = true end
                    end
                    if instance.Fluctuated then
                        if instance.Dot >= 0 and not instance.Parryable then instance.Parryable = true end
                    end

                    if instance.ZoomiesFluctuated and instance.ZoomiesParryable and instance.ZoomiesDot < instance.LastDot and not instance.DebounceZoomiesDecreaseParryable then
                        instance.DebounceZoomiesDecreaseParryable = true
                        instance.ZoomiesDecreaseParryable = true
                    end
                    if instance.Fluctuated and instance.Parryable and instance.Dot < instance.LastDot then
                        instance.DecreaseParryable = true
                    end

                    if instance.Velocity.Magnitude > instance.HighestVelocity.Magnitude then
                        instance.HighestVelocity = instance.Velocity
                    end

                    if Humanoids.LocalPlayer and Humanoids.LocalPlayer.RootPart then
                        instance.DistanceFromTarget = (Humanoids.LocalPlayer.RootPart.Position - instance.Position).Magnitude
                    end
                    if Humanoids.Target and Humanoids.Target.RootPart then
                        instance.DistanceFromTarget = (Humanoids.Target.RootPart.Position - instance.Position).Magnitude
                    end
                    if Humanoids.From and Humanoids.From.RootPart then
                        instance.DistanceFromFrom = (Humanoids.From.RootPart.Position - instance.Position).Magnitude
                    end
                    if Humanoids.LastTarget then
                        instance.DistanceFromLastTarget = (Humanoids.LastTarget.RootPart.Position - instance.Position).Magnitude
                    end
                    if Humanoids.LastActualTarget then
                        instance.DistanceFromLastActualTarget = (Humanoids.LastActualTarget.RootPart.Position - instance.Position).Magnitude
                    end
    ...
    if typeof(State.ParryAttemptKey) ~= "string" and typeof(RemoteSignals.ParryAttempt) ~= "Instance" then
        notifyFn("Executor does not support function.", 15)
        return nil
    end
    ...
    -- // REUSABLE CALLBACK FACTORIES //
    local Callbacks = {}

    function Callbacks.getPing()
        local ping
        pcall(function() ping = (LocalPlayer:GetNetworkPing() * 2) or (game.Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000) end)
        return ping or 0.05
    end

    function Callbacks.resolveTargetPlayer()
        local Humanoids = { LocalPlayer = Utils:GetHumanoid(LocalPlayer.Character) }
        if not Humanoids.LocalPlayer or not Humanoids.LocalPlayer.RootPart then return nil end
    ...
    function Callbacks.buildParryArgs(targetPlayer)
        local parryArgs = {}
    ...
    function Callbacks.manualParry()
        local Humanoids = { LocalPlayer = Utils:GetHumanoid(LocalPlayer.Character) }
        ...
        if not isSpamming then
            local targetPlayer = Callbacks.resolveTargetPlayer()
            parryArgs = Callbacks.buildParryArgs(targetPlayer)
    ...
    function Callbacks.autoParryToggle(bool)
    ...
    function Callbacks.autoSpamParryToggle(bool)
    ...
    function Callbacks.autoConfigToggle(bool, setRangeFn, setDirectPointFn)
        ...
        SignalWrapper["Auto-Config"] = RunService.PostSimulation:Connect(TEMP_NO_VIRTUALIZE(function()
            ...
            if isDungeon then
                setRangeFn(0.3)
                setDirectPointFn(-0.25)
            else
                setRangeFn(0.35 + 0.05 * (ping / 0.05))
                setDirectPointFn(0)
            end
        end))
    end

    function Callbacks.blockSpamParryToggle(bool)
    ...
    return {
        ...
        GUI = GUI,
        Callbacks = Callbacks,
        TEMP_NO_VIRTUALIZE = TEMP_NO_VIRTUALIZE,
        NotifyName = "Infinixity",
